document.addEventListener('DOMContentLoaded', function () {
    ...
    fetch("https://v2.api.noroff.dev/blog/posts/Christian_Westby")
        ...
        .then(function appendData(data) {
            ...
            // Legg til event listeners for å håndtere klikk på post-lenkene
            document.querySelectorAll('.single-post-link').forEach(link => {
                link.addEventListener('click', function (event) {
                    event.preventDefault();
                    const postId = this.href.split('?id=')[1];
                    window.location.href = `./account/post/singlepost.html?id=${postId}`;
                });
            });
        })
        .catch(error => {
